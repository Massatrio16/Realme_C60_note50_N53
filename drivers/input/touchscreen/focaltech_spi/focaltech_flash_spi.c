...
#define FTS_FW_NAME_PREX_WITH_REQUEST               "focaltech_ts_fw_ft8057p_ld"
#define FTS_HWID_CTP_DRIVER                         "FT_8057P"
...
//xiazhiping@BSP.TP.Function add for hardinfo start
static char firmware_version[25] = {'H','U','L','K','_','L','C','E','_','H','K','C','_','F','T','0','7'};
//xiazhiping@BSP.TP.Function add for hardinfo end
...
static int fts_enter_into_boot(void)
{
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
        ...
        } else {// WuXiaolong@BSP.TP 2024/1/26 Add for ITO enter test/normal environment fail
...
//#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0))
    ...
    *file_buf = (u8 *)vmalloc(file_len);
    ...
    //danchuanmin@BSP.TP 2024/2/18 go here for the userdebug version
    #ifdef USER_DEBUG
...
void  get_tp_fw_ver(void)
{
    ...
    if(fwver)
    {
        ...
        firmware_version[17] = buf[0];
        firmware_version[18] = buf[1];
        get_hardware_info_data(HWID_CTP_FW_INFO,firmware_version);
        get_hardware_info_data(HWID_CTP_DRIVER, FTS_HWID_CTP_DRIVER);
    }
}
//xiazhiping@BSP.TP.Function add for hardinfo end
